...
class TodoModel {
  constructor (key) {
    ...
    this.todos = []
    this.onChanges = []
    this.FIREBASE_PROPERTY = 'xxxx_xxxxxx'
    const config = {
  ...
  setUpDBaaS (userId) {
    ...
    this.FIREBASE_PROPERTY = `users/${userId}`
    this.database.ref(this.FIREBASE_PROPERTY)
    .on('value', (snapshot) => {
  ...
  toggleAll (checked) {
    ...
    this.todos.forEach((todo) => this.database
      .ref(`${this.FIREBASE_PROPERTY}/${todo.id}`)
      .set(todo)
    )
  }
