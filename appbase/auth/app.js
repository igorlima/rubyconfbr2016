...
class TodoModel {
  ...
  setUpDBaaS () {
    ...
    this.appbaseRef.searchStream({
      ...
      if (_deleted) {
        ...
        todo ? Object.assign(todo, _source) : this.todos.push(_source)
      }
  ...
  setUpLogin () {
    hello.init({facebook: 234391830287577}, {
      redirect_uri: '#'
    }).on('auth.login', (auth) => {
      // Call user information, for the given network
      hello(auth.network).api('me').then((r) => {
        this.ES_TYPE = `${r.id}${r.email ? r.email.match(/[a-zA-Z0-9]/g, '').join('') : ''}`
        // this.ES_TYPE = `${r.id}`
        this.setUpDBaaS()
      });
    })
  ...
  addTodo (title) {
    ...
    // optimistc logic
    this.todos = this.todos.concat(jsonObject)
    this.inform()

    // broadcast all changes
    this.appbaseRef && this.appbaseRef.index({
      type: this.ES_TYPE,
      id: id,
      body: jsonObject
  ...
  destroy (todo) {
    ...
    // broadcast all changes
    this.appbaseRef && this.appbaseRef.delete({
      type: this.ES_TYPE,
      id: todo.id
    }).on('data', function(response) {
      console.log(response)
...
let render = () => {
  let islogin = !!model.appbaseRef
  ReactDOM.render(
    islogin ? <TodoApp model={model}/> : <button className="loginBtn" onClick={login}>Please sign in first</button>,
